              ...
              >
                ...
                <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center ${
                  msg.role === 'assistant' 
                    ...
                    : darkMode ? 'bg-[#2F2F2F]' : 'bg-slate-200'
                }`}>
                  {msg.role === 'assistant' ? (
                    <Sparkles className="w-4 h-4 text-white" />
                  ) : (
                    <User className={`w-4 h-4 ${darkMode ? 'text-white' : 'text-slate-600'}`} />
                  )}
                </div>
                ...
                <div className={`flex-1 min-w-0 ${msg.role === 'user' ? 'text-right' : ''}`}>
                  <div className={`inline-block max-w-[85%] p-4 rounded-2xl ${
                    ...
                    >
                      ...
                      <p className="text-sm font-medium text-left">{msg.content}</p>
                    )}
                  </div>
                  ...
                  {msg.role === 'assistant' && !msg.isError && (
                    <div className={`flex items-center gap-2 mt-2 ${msg.role === 'user' ? 'justify-end' : ''}`}>
                      <button className={`p-1.5 rounded-md ${darkMode ? 'hover:bg-[#2F2F2F]' : 'hover:bg-slate-100'}`}>
                        <Copy className={`w-4 h-4 ${darkMode ? 'text-[#8A8A8A]' : 'text-slate-400'}`} />
                      </button>
                      <button className={`p-1.5 rounded-md ${darkMode ? 'hover:bg-[#2F2F2F]' : 'hover:bg-slate-100'}`}>
                        <ThumbsUp className={`w-4 h-4 ${darkMode ? 'text-[#8A8A8A]' : 'text-slate-400'}`} />
                      </button>
                      <button className={`p-1.5 rounded-md ${darkMode ? 'hover:bg-[#2F2F2F]' : 'hover:bg-slate-100'}`}>
                        <ThumbsDown className={`w-4 h-4 ${darkMode ? 'text-[#8A8A8A]' : 'text-slate-400'}`} />
                      </button>
                    </div>
                  )}
                </div>
